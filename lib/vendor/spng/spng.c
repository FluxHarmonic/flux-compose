...
#if defined(SPNG_ARM)
static uint32_t expand_palette_rgba8_neon(unsigned char *row, const unsigned char *scanline,
                                          const unsigned char *plte, uint32_t width);
/*static uint32_t expand_palette_rgb8_neon(unsigned char *row, const unsigned char *scanline, const
 * unsigned char *plte, uint32_t width);*/
#endif
#endif
#endif
...
static void expand_row(unsigned char *row, const unsigned char *scanline,
                       const union spng__decode_plte *decode_plte, uint32_t width, int fmt) {
  uint32_t i = 0;
  unsigned char *px;
  unsigned char entry;
...
static void unpack_scanline(unsigned char *out, const unsigned char *scanline, uint32_t width,
  ...
  while (!(ret = read_header(ctx))) {
    ...
    } else /* Arbitrary-length chunk */
    {
      ...
      } else /* Unknown chunk */
      {
        ...
        void *buf =
            spng__realloc(ctx, ctx->chunk_list, ctx->n_chunks * sizeof(struct spng_unknown_chunk));
        if (buf == NULL)
          return SPNG_EMEM;
        ctx->chunk_list = buf;
  ...
  if (f.indexed) {
    ...
    for (i = 0; i < 256; i++) {
      ...
      if (fmt == SPNG_FMT_RGB8 &&
          ihdr->bit_depth == 8) { /* Working with 3 bytes at a time is more of an ARM thing */
...
static int write_chunks_before_idat(spng_ctx *ctx) {
  ...
  if (ctx->stored.iccp) {
    ...
    ret = compress2(buf, &dest_len, (void *)ctx->iccp.profile, (uLong)ctx->iccp.profile_len,
...
static int encode_scanline(spng_ctx *ctx, const void *scanline, size_t len) {
  ...
  filter = get_best_filter(ctx->prev_scanline, ctx->scanline, scanline_width, ctx->bytes_per_pixel,
                           f.filter_choice);
...
void *spng_get_png_buffer(spng_ctx *ctx, size_t *len, int *error) {
  ...
  if (ctx->state != SPNG_STATE_IEND) {
    if (ctx->state >= SPNG_STATE_ENCODE_INIT)
